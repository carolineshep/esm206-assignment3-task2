...
sw_split_name <- starwars %>% # Start with starwars data
  separate(name, # Separate based on name column
...
```{r}
# Your code here:
sw_skin_sep <- starwars %>% # Start with starwars data
  separate(col = skin_color, # Separate the skin_color column
           into = c("skin_1", "skin_2"), # Separate into two columns: skin_1 and skin_2
           sep = ",", # Separate at the first comma
           extra = "merge", # If additional spaces exist, merge all into the second column
...
mpg_model_class <- mpg %>% # Start with  mpg data
  unite(col = model_class, # Create a new column with the name 'model_class'
        c("model", "class"), # Combine the model and class columns
        sep = "-", # Separate at the first dash
        remove = FALSE) # do not remove the original column
...
ymd(20201118) # Concert to date class
...
```{r}
# Your code here: 
ymd(19970506) # Convert to date class

ymd("1997/05/06") # Convert to date class
...
some_dates <- c(05061997, 06062007, 01271997) # Create a new vector of dates without spaces

some_dates_converted <- lubridate::mdy(some_dates) # Converts these dates, save as some_dates_converted

class(some_dates) # Check the class
...
gasol_hookshots <- lakers %>%  # Start with lakers data
  filter(player == "Pau Gasol", type == "hook") %>% # Filter to find where player is "Pau Gasol" and shot type is "hook"
  mutate(date_new = ymd(date)) %>% # Add a new date class
  mutate(month_new = month(date_new)) # Add new column for month

hookshots_recorded <- gasol_hookshots %>% # Find total shots recorded
  count(month_new)
```
...
storms_danger <- storms %>% # Start with storms data
  mutate(danger = case_when( # Add a new column for danger
    status %in% c("hurricane") ~ "high", # Danger status is 'high' if storm type is 'hurricane'
    status %in% c("tropical storm") ~ "moderate", # Danger status is 'moderate' if storm type is 'tropical storm'
    status %in% c("tropical depression") ~ "moderate")) # Danger status is 'moderate' if storm type is 'tropical depression'
